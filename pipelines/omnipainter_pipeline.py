...
class UniStyLatentConsistencyModelPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # timesteps = self.scheduler.timesteps[45:46]
        ...
        style = self.image_processor.preprocess(style, height=height, width=width)
        # fix_step_index = timesteps[-1]
        # print('fix_step_index is :', fix_step_index)
        # exit()
        ...
        mutual_latents = torch.randn_like(sty_latents).to(device=device, dtype=sty_latents.dtype)
        ...
        # # Filter parameters
        # filter_shape = sty_latents.shape
        # sty_latents = sty_latents.to(dtype=torch.float32)
        # method = 'butterworth'  # Can also be 'gaussian', 'ideal', or 'box'
        # n = 4
        # d_s = 0.25

        # low_freq_filter = get_freq_filter(
        #     filter_shape,
        #     device=device,
        #     filter_type=method,
        #     n=n,
        ...
        # high_freq_filter = 1 - low_freq_filter

        # # Apply filter
        # style_latents_freq = torch.fft.fftn(sty_latents, dim=(-2, -1))
        # style_latents_freq = torch.fft.fftshift(style_latents_freq, dim=(-2, -1))

        # # frequency filtering
        # style_latents_freq_low = style_latents_freq * low_freq_filter
        # style_latents_freq_high = style_latents_freq * high_freq_filter

        # # inverse fft
        # style_latents_freq = style_latents_freq_low * 0.5 + style_latents_freq_high
        # style_latents_freq = torch.fft.ifftshift(style_latents_freq, dim=(-2, -1))
        # sty_latents = torch.fft.ifftn(style_latents_freq, dim=(-2, -1)).real
        # sty_latents = sty_latents.to(dtype=dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #     sty_latents_mean = sty_latents.mean(dim=1, keepdim=True)
                #     sty_latents_std = sty_latents.std(dim=1, keepdim=True)

                #     mutual_latents_mean = mutual_latents.mean(dim=1, keepdim=True)
                #     mutual_latents_std = mutual_latents.std(dim=1, keepdim=True)

                #     # mutual_latents = sty_latents_std * (mutual_latents - mutual_latents_mean) / mutual_latents_std + sty_latents_mean
                #     mutual_latents = mutual_latents - mutual_latents_mean * 0.6 + sty_latents_mean * 0.6

                # mutual_latents = mutual_latents.to(prompt_embeds.dtype)
                # latent_timestep = torch.tensor([t], device=device)
                # sty_latents, sty_clean_latents = self.prepare_latents(
                #     style, latent_timestep, device, dtype, denoise_model, generator
                # ) 
                model_input = torch.cat([sty_latents, mutual_latents], dim=0)
                ...
                # style_noise_pred, mutual_noise_pred = model_pred.chunk(2, dim=0)
                # mutual_latents, pred_x0_mutual = self.scheduler.step(mutual_noise_pred, t, mutual_latents, **extra_step_kwargs, return_dict=False)
                # _,              pred_x0_style = self.scheduler.step(style_noise_pred, t, sty_latents, **extra_step_kwargs, return_dict=False)

                latents_cat, denoised_cat = self.scheduler.step(model_pred, t, model_input, **extra_step_kwargs, return_dict=False)
                _, mutual_latents = latents_cat.chunk(2, dim=0)
                pred_x0_style, pred_x0_mutual = denoised_cat.chunk(2, dim=0)
        ...
        pred_x0 = torch.cat([sty_clean_latents, pred_x0_style, pred_x0_mutual], dim=0)
        ...
        # print('do_denormalize', do_denormalize)
        output_type = 'np'
