...
def adain_op(hidden_states):
    ...
    # style_mean = style_hidden_states.mean(dim=(2, 3), keepdim=True)
    # style_std = style_hidden_states.std(dim=(2, 3), keepdim=True)

    # source_mean = source_hidden_states.mean(dim=(2, 3), keepdim=True)
    # source_std = source_hidden_states.std(dim=(2, 3), keepdim=True)

    # source_hidden_states = style_std * (source_hidden_states - source_mean) / source_std + style_mean
    source_hidden_states = source_hidden_states - source_mean + style_mean
...
class ResnetBlock2D(nn.Module):
    ...
    def forward(
        ...
        # hidden_states = adain_op(hidden_states)
        hidden_states = self.conv1(hidden_states, scale) if not USE_PEFT_BACKEND else self.conv1(hidden_states)
        # hidden_states = adain_op(hidden_states)
        # if self.res_type == "UpBlock2D":
        #     hidden_states = adain_op(hidden_states)
